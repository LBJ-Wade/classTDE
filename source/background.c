...
int background_w_fld(
  ...
  double zt = 1.5;
  double dz = 0.9;
  double w1 = pba->wa_fld;
  double w0 = pba->w0_fld;
  double ttt = (pba->a_today/a - zt - 1)/dz;
  double dtttda = -pow(pba->a_today/a,2)/dz;
  double d2tttda2 = 2*pow(pba->a_today/a,3)/dz;
  double HypTan = tanh(ttt);
  double dWda = -0.5*(1-pow(HypTan,2))*dtttda;
  double d2Wda2 = -0.5*(-2*HypTan*(1-pow(HypTan,2))*pow(dtttda,2) + (1-pow(HypTan,2))*d2tttda2);
  if (a>0)
  {
    *w_fld = w0 + 0.5*(w1-w0)*(HypTan+1) + log(a/pba->a_today)*a/pba->a_today*dWda;
    *dw_over_da_fld = 0.0;
    /***dw_over_da_fld = (2+log(a/pba->a_today))*dWda + log(a/pba->a_today)*a/pba->a_today*d2Wda2;**/
  }
  if (a==0)
  {
    *w_fld = -2.0;
  ...
  /***integral_fld = 3.*(1.+*w_fld)*log(pba->a_today/a)+2.07867;**/
  *integral_fld = -log(a/pba->a_today)*3.*(1. + pba->w0_fld + 0.5*(w1-w0)*(HypTan+1));
